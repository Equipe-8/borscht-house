...
export const CartContextProvider = ({ children }: IDefaultProviderProps) =>
{
  ...
  const searchList = products.filter(product => product.name.toLowerCase().includes(search.toLowerCase()) || product.country.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <CartContext.Provider value={{ products, setProducts, carts, setCarts, search, setSearch, searchList, searchCart, toAdd }}>
      { children }
